...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action']??'')==='send') {
    try {
        $pdo->prepare("INSERT INTO referrals (sender_id,receiver_id,referred_name,referred_business_type,phone,email,estimated_value,notes,status,created_at) VALUES (?,?,?,?,?,?,?,?,'sent',NOW())")
        ...
        try{ $pdo->prepare("UPDATE voocoin_balances SET balance = balance + 50 WHERE user_id = ?")->execute([$uid]); } catch(Throwable $e){}
        try{ $pdo->prepare("INSERT INTO coin_transactions(user_id,amount,type,description,created_at)VALUES(?,50,'earn','Referral given',NOW())")->execute([$uid]);}catch(Throwable $e){}
...
$members=$pdo->prepare("SELECT id,business_name,city FROM users WHERE id!=? AND status='active' ORDER BY business_name LIMIT 300");$members->execute([$uid]);$members=$members->fetchAll();
      ...
      <div class="tab-pane fade show active" id="st">
        ...
        <div class="card bg-dark border-secondary mb-2 p-3">
          <div class="d-flex justify-content-between">
            ...
            <div class="text-muted small"><?=htmlspecialchars($r['referred_business_type']??'')?> · <?=htmlspecialchars($r['phone']??'')?></div>
            <?php if(isset($r['rname']) && $r['rname']): ?><div class="small text-warning">→ <?=htmlspecialchars($r['rname'])?></div><?php endif; ?>
            <?php if(isset($r['estimated_value']) && $r['estimated_value']): ?><div class="small text-muted">₹<?=number_format($r['estimated_value'])?></div><?php endif; ?></div>
            <span class="badge bg-<?=match($r['status']??'sent'){'done','completed','closed','sent'=>'success','rejected'=>'danger',default=>'warning'}?>"><?=ucfirst($r['status']??'sent')?></span>
          </div>
        </div>
        <?php endforeach; ?>
      </div>
      <div class="tab-pane fade" id="rv">
        ...
        <div class="card bg-dark border-secondary mb-2 p-3">
          <div class="d-flex justify-content-between">
            ...
            <?php if(isset($r['sname']) && $r['sname']): ?><div class="small text-info">From: <?=htmlspecialchars($r['sname'])?></div><?php endif; ?></div>
            <span class="badge bg-<?=match($r['status']??'sent'){'done','completed','closed','sent'=>'success','rejected'=>'danger',default=>'warning'}?>"><?=ucfirst($r['status']??'sent')?></span>
          </div>
        </div>
        <?php endforeach; ?>
