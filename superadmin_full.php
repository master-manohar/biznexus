...
<div class="row g-4 mb-4">
    <div class="col-md-12">
        <div class="sa-card">
            <h6 style="color:#FFD700;margin-bottom:15px;"><i class="fas fa-robot me-2"></i> Unique Agent Fleet Status</h6>
            <div class="row g-3">
                <?php 
                $summary = $pdo->query("SELECT task_type, COUNT(*) as total, 
                    SUM(CASE WHEN status='running' THEN 1 ELSE 0 END) as active,
                    MAX(updated_at) as last_act 
                    FROM agent_tasks GROUP BY task_type")->fetchAll(PDO::FETCH_ASSOC);
                foreach($summary as $s): 
                    $icon = ['social_posting'=>'fa-share-alt','outreach_marketing'=>'fa-envelope','seo_dominance'=>'fa-search','profile'=>'fa-user-plus'][$s['task_type']] ?? 'fa-cog';
                ?>
                <div class="col-md-3">
                    <div style="background:rgba(255,255,255,.03);padding:12px;border-radius:10px;border:1px solid #2a2a3a;">
                        <div style="display:flex;justify-content:space-between;align-items:start;">
                            <i class="fas <?= $icon ?> fa-lg" style="color:#4488ff;"></i>
                            <?php if($s['active'] > 0): ?><span class="pill pill-blue" style="font-size:.5rem;animation:pulse 2s infinite;">RUNNING</span><?php endif; ?>
                        </div>
                        <div style="margin-top:10px;">
                            <div style="font-size:.75rem;font-weight:700;color:#fff;"><?= strtoupper(str_replace('_',' ',$s['task_type'])) ?></div>
                            <div style="font-size:.65rem;color:#666;">Total Tasks: <?= $s['total'] ?> | Active: <?= $s['active'] ?></div>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
