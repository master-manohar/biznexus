    ...
    <div class="col-lg-9">
      ...
      <?php else: ?>
                ...
                <div class="card-price">
                  <?php if (!empty($l['price']) && $l['price'] > 0): ?>
                    ...
                    <?php if(!empty($l['price_type'])): ?>
                      <span class="price-label">/<?= htmlspecialchars($l['price_type']) ?></span>
                    <?php endif; ?>
                  <?php endif; ?>
                </div>
                <div class="card-meta">
                  <?php if (!empty($l['seller_avatar'])): ?>
                    <img src="<?= htmlspecialchars($l['seller_avatar']) ?>" class="avatar-sm" alt="Seller"/>
                  <?php else: ?>
                    <div class="avatar-placeholder"><i class="fa fa-user"></i></div>
                  <?php endif; ?>
                  <span><?= htmlspecialchars($l['seller_name'] ?? 'Unknown User') ?></span>
                </div>
              </div>

              <!-- Footer -->
              <div class="card-footer-inner">
                <a href="/marketplace/view.php?id=<?= $l['id'] ?>" class="btn-view">View details</a>
                <?php if (!empty($l['contact_email'])): ?>
                <a href="mailto:<?= htmlspecialchars($l['contact_email']) ?>" class="btn-contact">Contact</a>
                <?php else: ?>
                <button class="btn-contact" onclick="alert('Contact not provided.')">Contact</button>
                <?php endif; ?>
              </div>
            </div>
          </div>
        ...
        <div class="pagination-wrap">
          <?php if($page > 1): ?>
            <a href="<?= pagUrl($page-1) ?>" class="page-btn">Prev</a>
          <?php endif; ?>
          
          <?php for($i=1; $i<=$total_pages; $i++): ?>
            <a href="<?= pagUrl($i) ?>" class="page-btn <?= $i===$page?'active':'' ?>"><?= $i ?></a>
          <?php endfor; ?>
          
          <?php if($page < $total_pages): ?>
            <a href="<?= pagUrl($page+1) ?>" class="page-btn">Next</a>
          <?php endif; ?>
        </div>
        <?php endif; ?>
